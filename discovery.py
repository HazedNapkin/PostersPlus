...
_SASH_TYPES: dict[str, str] = {
    ...
    "status_cinema":   "alert",
    "status_streaming": "alert",
    "status_physical": "alert",
    "status_production": "alert",
    "status_returning": "alert",
    "status_cancelled": "alert",
}
...
def _evaluate_slot(slot: str, meta: DiscoveryMeta) -> str | None:
    ...
    if slot == "short_film" and meta.is_short_film: return _STRUCTURAL_LABELS["short_film"]
    if slot == "mini_series" and meta.is_mini_series: return _STRUCTURAL_LABELS["mini_series"]
    if slot == "binge_ready" and meta.is_binge_ready: return _STRUCTURAL_LABELS["binge_ready"]

    if slot == "release_status":
        return meta.release_status  # already a display string or None
    
    if slot.startswith("status_"):
        expected_status = slot.split("_", 1)[1].lower()
        if meta.release_status and meta.release_status.lower() == expected_status:
